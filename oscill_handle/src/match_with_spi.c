...
static int SearchFirstMatch(unsigned char *oscill_data_buffer) {
  ...
  for (i = 0;; i++) {
    ...
    sprintf(des_file_name, WORK_PATH "data_spi/data_capture_interval_%d.bin",
            i);
    ...
    int cmp_result =
        MemoryCompare(oscill_data_buffer, file_contet + SPI_HEAD_LEN,
                      get_size - SPI_HEAD_LEN);
    if (cmp_result == 1) {
      return i;
    }
  }
...
static int FindSubstring(char *source_file_name, FILE *fp_log,
                         int *cur_spi_file_count) {
  int first_offset;
  unsigned char file_contet[SPI_FILE_SIZE];
  ...
  if (*cur_spi_file_count < 0) {
    ...
    for (spi_file_num = 0; spi_file_num <= READ_INTERVAL_TIME; spi_file_num++) {
      ...
      int cmp_result = MemoryCompare(sou_buffer, file_contet + SPI_HEAD_LEN,
                                     get_size - SPI_HEAD_LEN);
      if (cmp_result == 1) {
        *cur_spi_file_count += spi_file_num;
        return 1;
...
int MatchData(int oscill_file_num) {
  int i;
  ...
  for (i = 0; i < oscill_file_num; i++) {
    ...
    if (ret == -1) {
      ...
      sprintf(
          log_string,
          "oscillo_valid_data_%d.bin <-------> data_capture_interval_%d.bin\n",
          i, cur_spi_file_count);
