...
int DataFormatTransfrom() {
  ...
  unsigned char *read_buffer = (unsigned char *)malloc(
      GET_DATA_TIME * SINGLE_READ_MAX_LEN * sizeof(char));
  int i;
  for (i = 0;; i++) {
    ...
    sprintf(valid_file_name,
            WORK_PATH "data_oscillo_a\\oscillo_valid_data_%d.bin", i);
    ...
    if ((data_size = fread(read_buffer, 1, GET_DATA_TIME * SINGLE_READ_MAX_LEN,
                           fp_source)) != 0) {
      int count = 0;
...
//     long long data_wrap = 0;
//     size_t data_size;
//     unsigned char *read_buffer = (unsigned char *)malloc(GET_DATA_TIME *
//     SINGLE_READ_MAX_LEN * sizeof(char));
//     int oscill_file_num;
//     int i;
...
//         char origin_file_name[FILE_NAME_MAX_LEN];
//         char valid_file_name[FILE_NAME_MAX_LEN];
//         sprintf(origin_file_name, WORK_PATH"origin_data/rigol_data_%d.bin",
//         i);
//         sprintf(valid_file_name,
//         WORK_PATH"data_oscillo_b/oscillo_valid_data_%d.bin", i);
//         fp_source = fopen(origin_file_name, "rb");
//         if(fp_source == NULL)
//         {
//             break;
...
//         if((data_size = fread(read_buffer, 1, GET_DATA_TIME *
//         SINGLE_READ_MAX_LEN, fp_source)) != 0)
//         {
//             int count = 0;
//             int wrap_4bit_count = 0;
//             long long *write_slip = (long long *)read_buffer;

//             while(((*(read_buffer + count) & 0x10) == 0) && (count <
//             data_size))
//             {
//                 count++;
//             }
...
//                 {
//                     data_wrap |= (long long)(*(read_buffer + count) &
//                     data_mask) << (4 * wrap_4bit_count);
