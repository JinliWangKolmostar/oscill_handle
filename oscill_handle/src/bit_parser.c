...
static const char mistake_file_name[FILE_NAME_MAX_LEN] =
    WORK_PATH "mistake_log.txt";

int CompareBitDiff(unsigned char *sou_buf, unsigned char *des_buf, int buf_size,
...
int AnalyzeAndCorrect(int file_num) {
  int i;
    ...
    if (wrong_file_nums[i] == 0) {
      ...
      if (AllChannelsMatched(&iq_count, buf_size) == 1) {
        ...
        sprintf(log_char, "four channels IQ: I0:%d   I1:%d    Q0:%d    Q1:%d\n",
                iq_count.i0_count, iq_count.i1_count, iq_count.q0_count,
                iq_count.q1_count);
        fwrite(log_char, 1, strlen(log_char), wrong_fp);
        ...
        sprintf(log_char, "four channels IQ: I0:%d   I1:%d    Q0:%d    Q1:%d\n",
                iq_left_count.i0_count, iq_left_count.i1_count,
                iq_left_count.q0_count, iq_left_count.q1_count);
        fwrite(log_char, 1, strlen(log_char), wrong_fp);
        if (AllChannelsMatched(&iq_left_count, buf_size) == 1) {
          ...
          sprintf(log_char,
                  "four channels IQ: I0:%d   I1:%d    Q0:%d    Q1:%d\n",
                  iq_right_count.i0_count, iq_right_count.i1_count,
                  iq_right_count.q0_count, iq_right_count.q1_count);
          fwrite(log_char, 1, strlen(log_char), wrong_fp);
  ...
  printf("result:total file number:%d, match: %d, mismatch: %d, ", file_num,
         file_num - wrong_number, wrong_number);
